...
def is_match_storage(lineup_val, mat_val):
    if pd.isna(lineup_val) or str(lineup_val).strip() == "":
        return True
    l_str = str(lineup_val).lower().replace(' ', '').replace('gb','').replace('g','').replace('tb','t').replace('t','t')
    m_str = str(mat_val).lower().replace(' ', '').replace('gb','').replace('g','').replace('tb','t').replace('t','t')
    return l_str in m_str
...
current_brand = None
df_out = df_lineup.copy().astype(object)
...
for i, row in df_out.iterrows():
    ...
    mask_hdd = df_mat['HDD'].apply(lambda x: is_match_storage(storage, x))
    ...
    for _, m_row in matched.iterrows():
        ...
        if pd.notna(min_p) and min_p < best_price:
            ...
            best_model = str(m_row['Наименование товара'])
