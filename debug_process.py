...
for i, row in df_out.iterrows():
    ...
    print(f"Row {i} - matched {len(matched)}")
    
    if matched.empty:
        df_out.iat[i, 2] = 'N/A'
        df_out.iat[i, 3] = 'N/A'
        print("  Set N/A due to empty match")
        continue
    ...
    if current_brand.lower() == 'lenovo':
        ...
        print(f"  Lenovo rule applied. Now {len(matched)} candidates. Max stock {max_stock}")
    ...
    if best_price == np.inf:
        df_out.iat[i, 2] = 'N/A'
        df_out.iat[i, 3] = 'N/A'
        print("  Set N/A due to inf price")
    else:
        df_out.iat[i, 2] = best_price
        df_out.iat[i, 3] = best_model
        print(f"  Set Price {best_price}")
        
df_out.to_excel(os.path.join(prices_dir, "Line up monitoring_filled.xlsx"), index=False)
print("Finished!")
